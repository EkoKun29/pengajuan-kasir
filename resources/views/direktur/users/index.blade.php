...
<div x-data="{ 
    ...
    <div class="bg-white rounded-xl shadow-sm border border-gray-100 p-6">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h1 class="text-2xl font-bold text-gray-900">{{ auth()->user()->role === '' ? 'Manajemen User' : 'Daftar User' }}</h1>
                <p class="text-sm text-gray-500 mt-1">{{ auth()->user()->role === 'direktur' ? 'Kelola akun pengguna sistem' : 'Informasi pengguna sistem' }}</p>
            </div>
            <div class="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
                {{-- Search Form --}}
                <form method="GET" action="{{ auth()->user()->role === 'direktur' ? route('direktur.users.index') : route('keuangan.users.index') }}" class="flex gap-2">
                    <div class="relative">
                ...
                @forelse($users as $user)
                    <tr class="hover:bg-gray-50 transition-colors">
                        ...
                        <td class="px-6 py-4 whitespace-nowrap">
                            @php $color = $user->role === 'direktur' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800'; @endphp
                        ...
                        <td class="px-6 py-4 whitespace-nowrap text-sm space-x-2">
                            ...
                            {{-- Delete Button --}}
                            <form action="{{ route('direktur.users.destroy', $user) }}" method="POST" class="inline delete-form">
                                @csrf @method('DELETE')
                            ...
                            <div class="px-6 py-6">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="bg-gray-50 rounded-lg p-4">
                                        ...
                                        <dd class="mt-1">
                                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium {{ $user->role === 'direktur' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                                                {{ ucfirst($user->role) }}
                                            </span>
                                        </dd>
                                    </div>
                    ...
                    <div x-show="editModals[{{ $user->id }}]" x-cloak 
                        ...
                        <div @click.away="toggleModal('edit', {{ $user->id }})" 
                            ...
                            <div class="px-6 py-4">
                                <form method="POST" action="{{ route('direktur.users.update', $user) }}" class="space-y-4">
                                    @csrf @method('PUT')
                                    <div class="grid grid-cols-2 gap-4">
                                        <div>
                                        ...
                                        <div>
                                            ...
                                            <select name="role" class="w-full border-gray-300 rounded-lg px-3 py-2 focus:ring-2 focus:ring-blue-500 focus:border-blue-500" required>
                                                <option value="direktur" @selected($user->role==='direktur')>Direktur</option>
    ...
    <div x-show="openCreate" x-cloak class="fixed inset-0 bg-black/50 flex items-center justify-center z-50 p-4">
        <div @click.away="toggleModal('create')" class="bg-white rounded-xl shadow-xl w-full max-w-lg">
            ...
            <div class="px-6 py-4">
                <form method="POST" action="{{ route('direktur.users.store') }}" id="createForm" class="space-y-4">
                    @csrf
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-2">Nama</label>
